...
export const toQueryString = (params) => {
  ...
  if (params.latitude) {
    if (params.latitude[0]) {
      condition.push(
        `latitude__range=${params.latitude[0]},${params.latitude[1]}`
      );
    } else {
  ...
  if (params.longitude) {
    if (params.longitude[0]) {
      condition.push(
        `longitude__range=${params.longitude[0]},${params.longitude[1]}`
      );
    } else {
  ...
  if (TYPE_HOUSE_ENG[params.typeHouse]) {
    ...
    if (
      TYPE_HOUSE_ENG[params.typeHouse] === "redevelop" ||
      TYPE_HOUSE_ENG[params.typeHouse] === "house"
    ) {
      if (TYPE_HOUSE_DETAIL_ENG[params.typeHouseDetail]) {
        condition.push(
          `type_house_detail=${TYPE_HOUSE_DETAIL_ENG[params.typeHouseDetail]}`
        );
      }
    }
  }
  ...
  if (params.salePrice) {
    if (
      !params.salePrice.min &&
      !params.salePrice.max &&
      params.salePrice.text === "전체"
    ) {
    } else if (
      !params.salePrice.min &&
      !params.salePrice.max &&
      ...
      condition.push(
        `price__range=${params.salePrice.min},${params.salePrice.max}`
      );
    } else if (params.salePrice.min && !params.salePrice.max) {
  ...
  if (params.depositPrice) {
    if (
      !params.depositPrice.min &&
      !params.depositPrice.max &&
      params.depositPrice.text === "전체"
    ) {
    } else if (
      !params.depositPrice.min &&
      !params.depositPrice.max &&
      ...
      condition.push(
        `price_deposit__range=${params.depositPrice.min},${params.depositPrice.max}`
      );
    } else if (params.depositPrice.min && !params.depositPrice.max) {
